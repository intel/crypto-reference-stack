...
function generate_certs_fn () {
	...
	openssl ecparam -genkey -out $NGINX_INSTALL_DIR/certs/dsa-prime256v1.key -name prime256v1
	openssl req -x509 -new -key $NGINX_INSTALL_DIR/certs/dsa-prime256v1.key -out $NGINX_INSTALL_DIR/certs/dsa-prime256v1.crt -subj "/C=/ST=/L=/O=/OU=/CN=$(hostname)" 2> /dev/null
	openssl ecparam -genkey -out $NGINX_INSTALL_DIR/certs/dsa-secp384r1.key -name secp384r1
	openssl req -x509 -new -key $NGINX_INSTALL_DIR/certs/dsa-secp384r1.key -out $NGINX_INSTALL_DIR/certs/dsa-secp384r1.crt -subj "/C=/ST=/L=/O=/OU=/CN=$(hostname)" 2> /dev/null
}
...
function update_ssl_cipher_fn () {
	...
	if [[ "$au" == "ECDSA" ]]; then
		sed -i -r -e "s/ssl_certificate .*/ssl_certificate $sed_nginx_dir\/certs\/dsa-$cert_curve.crt;/" $nginx_config_file
		sed -i -r -e "s/ssl_certificate_key .*/ssl_certificate_key $sed_nginx_dir\/certs\/dsa-$cert_curve.key;/" $nginx_config_file
	elif [[ "$au" == "RSA" ]]; then
	...
	echo -e "\tssl_certificate:\t\t\t$(grep -Po 'ssl_certificate \K.*' $nginx_config_file)"
	echo -e "\tssl_certificate_key:\t\t$(grep -Po 'ssl_certificate_key \K.*' $nginx_config_file)\n"
}
...
TEMP=`getopt -o kslcCthu:f:e:vX:g --long kill-nginx,start-nginx,list-nginx,check-errors,clear-errors,tune-network,help,update-nginx-wp:,nginx-config-file:,nginx-qat-engine:,cipher-suite:,version,generate-certs,http,https -n 'Yikes' -- "$@"`
if [[ $? -ne 0 ]]; then
...
# Validate input parameters
if [[ "$nginx_qat_engine" -eq "-1" ]] && [[ "$update_nginx_wp" -eq "-1" ]] && [[ "$kill_nginx" -eq "0" ]] && [[ "$start_nginx" -eq "0" ]] && [[ "$list_nginx" -eq "0" ]] && [[ "$check_errors" -eq "0" ]] && [[ "$clear_errors" -eq "0" ]] && [[ "$tune_network" -eq "0" ]] && [[ -z "$ssl_cipher" ]] && [[ "$generate_certs" -eq "0" ]] && [[ "$use_https" -eq "-1" ]]; then
	echo -e "\nYou've to select at least one of [--kill-nginx | --start-nginx | --list-nginx | --check-errors | --clear-errors | --tune-network | --update-nginx-wp | --nginx-qat-engine | --generate-certs | --http | --https | --help | --version]"
	print_help_fn
	exit 0
fi
...
# using http will disable use of QAT_Engine
nginx_qat_engine_done=0
if [[ "$use_https" -eq "0" ]]; then
	nginx_qat_engine=0
	nginx_use_qat_engine_fn $nginx_qat_engine
	nginx_qat_engine_done=1
	sed -i -r -e 's/^(\s+)(ssl.*)/\1#_#\2/' $nginx_config_file
	sed -i -r -e 's/^(\s+)(listen [0-9]+)( ssl)?/\1\2/' $nginx_config_file
elif [[ "$use_https" -eq "1" ]]; then
	nginx_qat_engine=1
	nginx_use_qat_engine_fn $nginx_qat_engine
	nginx_qat_engine_done=1
	sed -i -r -e 's/#_#//' $nginx_config_file
	sed -i -r -e 's/^(\s+)(listen [0-9]+)( ssl)?/\1\2 ssl/' $nginx_config_file
fi


if [[ "$nginx_qat_engine" -ne "-1" ]] && [[ "$nginx_qat_engine_done" -eq "0" ]]; then
	nginx_use_qat_engine_fn $nginx_qat_engine
fi
